...
@app.get("/pos/calculateUserShiftsEOD/{company_name}/{date}/{allowedUser}")
async def calculateUserShiftsEOD(company_name: str, date: str, allowedUser: str):
    try:
        conn = get_db(company_name)
        cursor = conn.cursor()
        formatted_date = date.replace('.', '/')
        ...
        JOIN 
            inv ON inv.InvNo = invnum.InvNo
        WHERE 
            invnum.Date = '{formatted_date}' AND (invnum.EOD IS NULL OR invnum.EOD = '')
        """
        if allowedUser != "All":
            query = base_query + f" AND invnum.User = '{allowedUser}'"
        else:
